...
function buildStandingsPage({ title, standings, page, perPage }) {
  const start = page * perPage;
  const slice = standings.slice(start, start + perPage);

  const content =
    '\`\`\`' +
    slice.reduce((str, curr) => {
        return `${str}\n${curr.rank.padStart(2)}. ${curr.participant} - (${curr.score}-${curr.gamesplayed-curr.score} Record, ${curr.buchholz} Buchholz, Δ${curr.pointsdifference})`
      }, `${title}\n`) +
    '\`\`\`';
...
async function handleStandings(interaction, title, gid) {
  ...
  const message = await interaction.editReply({
    content: buildStandingsPage({ title, standings, page, perPage }),
